...
// Lê, a partir das salas do socket.io, quem está conectado agora numa call
// de canal de voz de servidor (sem precisar guardar estado à parte).
function voiceRoomOccupants(io, serverId, channelId) {
  ...
  if (roomSet) {
    for (const sid of roomSet) {
      ...
      if (peer && peer.userId) ids.push(Number(peer.userId));
    }
  }
  return [...new Set(ids)];
}

// Extrai @usuario de uma mensagem e resolve para ids de membros do servidor,
// para poder notificar quem foi mencionado mesmo sem estar vendo o canal.
function extractMentionedUserIds(content, serverId, authorId) {
  const matches = String(content || '').match(/@([a-zA-Z0-9_.]{2,32})/g) || [];
  const ids = new Set();
  for (const m of matches) {
    const username = m.slice(1);
    const user = User.findByUsername(username);
    if (user && Number(user.id) !== Number(authorId) && ServerModel.isMember(serverId, user.id)) {
...
function initSockets(io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('server-call:join', (data, callback) => {
      ...
      io.to(serverRoom(serverId)).emit('server-call:presence', { channelId, userIds: voiceRoomOccupants(io, serverId, channelId) });
    });
    socket.on('server-call:presence-snapshot', (data, callback) => {
      callback = typeof callback === 'function' ? callback : () => {};
      const serverId = Number(data?.serverId);
      if (!serverId || !ServerModel.isMember(serverId, userId)) return callback({ error: 'Não autorizado.' });
      const channels = Channel.listForUser(serverId, userId).filter(c => c.channel_type === 'voice');
      const presence = {};
      channels.forEach(c => { presence[c.id] = voiceRoomOccupants(io, serverId, c.id); });
    ...
    socket.on('server-call:leave', data=>{const sc=socket.data.serverCall;if(!sc)return;const room=sc.room;socket.leave(room);socket.to(room).emit('server-call:user-left',{userId,serverId:sc.serverId,channelId:sc.channelId});socket.data.serverCall=null;io.to(serverRoom(sc.serverId)).emit('server-call:presence',{channelId:sc.channelId,userIds:voiceRoomOccupants(io,sc.serverId,sc.channelId)});});
    ...
    socket.on('disconnect', () => {
      const sc=socket.data.serverCall; if(sc){socket.to(sc.room).emit('server-call:user-left',{userId,serverId:sc.serverId,channelId:sc.channelId});io.to(serverRoom(sc.serverId)).emit('server-call:presence',{channelId:sc.channelId,userIds:voiceRoomOccupants(io,sc.serverId,sc.channelId)});}
