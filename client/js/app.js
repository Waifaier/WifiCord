...
(function () {
  ...
  function guessMimeFromFileName(name) {
    ...
    const map = { jpg:'image/jpeg', jpeg:'image/jpeg', png:'image/png', gif:'image/gif', webp:'image/webp', avif:'image/avif', mp4:'video/mp4', mov:'video/quicktime', webm:'video/webm', mkv:'video/x-matroska', mp3:'audio/mpeg', wav:'audio/wav', ogg:'audio/ogg' };
    return map[ext] || '';
  }

  // Quando uma imagem/vídeo/áudio de mensagem não carrega (arquivo removido
  // do servidor, link quebrado, etc.), troca o player quebrado por um
  // cartão explicando o problema em vez de deixar o ícone de imagem
  // quebrada do navegador — mais claro pro usuário e mais bonito.
  window.__mediaLoadError = function (el) {
    try {
      if (el.dataset.mediaErrorHandled) return;
  ...
  function avatarHtml(entity) {
    ...
    const color = ps.profileColor || '#5865F2';
  ...
  // Fecha a gaveta de navegação (server-rail + sidebar) sozinha sempre que
  // a pessoa toca em algo clicável lá dentro no celular — configurações do
  // servidor, configurações da conta, loja, minigames, painel admin, sair,
  // etc. Antes só uns poucos pontos específicos (abrir servidor/canal/DM)
  // chamavam closeMobileNav(); qualquer outro botão dentro da gaveta
  // (settings-btn, server-settings-btn, store-btn, games-btn, admin-btn,
  // logout-btn, criar/entrar em servidor...) deixava a gaveta aberta por
  // cima da tela até a pessoa arrastar ela de volta na mão. Um único
  // listener delegado nos dois containers cobre todos eles de uma vez, sem
  // precisar caçar cada botão individualmente nem depender de quem mexer
  // nesse menu no futuro lembrar de fechar a gaveta manualmente.
  function bindMobileNavAutoClose() {
    function handler(e) {
      if (window.innerWidth > 680) return;
      if (!el.appScreen?.classList.contains('nav-open')) return;
      if (!e.target.closest('button, a, [data-server-id], [data-channel-id], [data-user-id]')) return;
      if (e.target.closest('#mobile-menu-btn')) return;
      closeMobileNav();
    }
    el.sidebar?.addEventListener('click', handler);
    el.serverRail?.addEventListener('click', handler);
  }

  // Fecha as abinhas (menu de servidores e lista de membros) sempre que a
  // pessoa clica em QUALQUER OUTRA COISA na tela — não só nos botões de
  // dentro delas (isso já é coberto por bindMobileNavAutoClose acima).
  // Cobre o caso de abrir configurações do servidor, clicar numa
  // mensagem etc. com a abinha ainda aberta por cima.
  function bindFloatingPanelsAutoClose() {
    document.addEventListener('click', function (e) {
      if (window.innerWidth > 680) return;
      if (el.appScreen?.classList.contains('nav-open') && !e.target.closest('#server-rail, #sidebar, #mobile-menu-btn')) {
        closeMobileNav();
      }
      if (el.membersPanel?.classList.contains('mobile-open') && !e.target.closest('#members-panel, #mobile-members-btn')) {
        closeMobileMembersDrawer();
      }
    });
  }
  ...
  // --- Papel de parede por conversa/canal ---------------------------------
  function wallpaperKey() {
    if (state.activeDMUserId) return 'dm:' + state.activeDMUserId;
    if (state.activeChannelId) return 'channel:' + state.activeChannelId;
    return null;
  }
  function wallpaperDefaults() {
    return { preset: 'none', customUrl: '', saturation: 100, contrast: 100, brightness: 100, dim: 35 };
  }
  function loadWallpaperSettings(key) {
    if (!key) return null;
    try { const raw = localStorage.getItem('wc-wallpaper:' + key); return raw ? JSON.parse(raw) : null; } catch (_) { return null; }
  }
  function saveWallpaperSettings(key, settings) {
    if (!key) return;
    try { localStorage.setItem('wc-wallpaper:' + key, JSON.stringify(settings)); } catch (_) {}
  }
  function applyWallpaperVars(target, settings) {
    const s = settings || {};
    target.style.setProperty('--wc-wp-sat', (Number(s.saturation ?? 100)) / 100);
    target.style.setProperty('--wc-wp-con', (Number(s.contrast ?? 100)) / 100);
    target.style.setProperty('--wc-wp-bri', (Number(s.brightness ?? 100)) / 100);
    target.style.setProperty('--wc-wp-dim', (Number(s.dim ?? 35)) / 100);
  }
  function applyChatWallpaper(key) {
    const layer = el.chatWallpaperLayer;
    if (!layer) return;
    const settings = key ? loadWallpaperSettings(key) : null;
    layer.className = 'chat-wallpaper-layer';
    layer.style.backgroundImage = '';
    if (!key || !settings || !settings.preset || settings.preset === 'none') {
      layer.classList.add('hidden');
      return;
    }
    layer.classList.remove('hidden');
    if (settings.preset === 'custom' && settings.customUrl) {
      layer.style.backgroundImage = 'url("' + settings.customUrl.replace(/"/g, '\\"') + '")';
    } else {
      layer.classList.add('wc-wallpaper-preset-' + settings.preset);
    }
    applyWallpaperVars(layer, settings);
  }
  function fillWallpaperModal() {
    const d = state.wallpaperDraft;
    if (!d) return;
    document.querySelectorAll('.wallpaper-preset-swatch').forEach(function (btn) {
      btn.classList.toggle('active', btn.dataset.wallpaperPreset === d.preset);
    });
    [['saturation', 100], ['contrast', 100], ['brightness', 100], ['dim', 35]].forEach(function (pair) {
      const input = $('wallpaper-' + pair[0]);
      const label = $('wallpaper-' + pair[0] + '-value');
      const v = d[pair[0]] ?? pair[1];
  ...
  function updateWallpaperPreview() {
    const layer = $('wallpaper-preview-layer');
    const d = state.wallpaperDraft;
    if (!layer || !d) return;
    layer.className = 'wallpaper-preview-layer';
    layer.style.backgroundImage = '';
    if (d.preset === 'custom' && d.customUrl) {
      layer.style.backgroundImage = 'url("' + d.customUrl.replace(/"/g, '\\"') + '")';
    } else if (d.preset && d.preset !== 'none') {
      layer.classList.add('wc-wallpaper-preset-' + d.preset);
    }
    applyWallpaperVars(layer, d);
  }
  function persistWallpaperDraft() {
    if (!state.wallpaperEditingKey) return;
    saveWallpaperSettings(state.wallpaperEditingKey, state.wallpaperDraft);
    applyChatWallpaper(wallpaperKey());
  }
  function openWallpaperModal() {
    const key = wallpaperKey();
    if (!key) { toast('Abra uma conversa ou canal primeiro.', 'error'); return; }
    state.wallpaperEditingKey = key;
    state.wallpaperDraft = Object.assign(wallpaperDefaults(), loadWallpaperSettings(key) || {});
    fillWallpaperModal();
  ...
  function bindEvents() {
    ...
    if (el.membersList) {
      el.membersList.addEventListener('click', function (e) {
        const item = e.target.closest('[data-member-id]');
        if (item) window.WCFeatures?.profile?.(item.getAttribute('data-member-id'));
      });
    }
    ...
    document.addEventListener('keydown', function (e) {
      ...
      // Esc fecha qualquer modal aberto (perfil, configurações, criar
      // servidor/canal, etc.), igual ao padrão do Discord.
      if (e.key === 'Escape' && el.modalOverlay && !el.modalOverlay.classList.contains('hidden')) {
        closeModals();
      }
    });
    ...
    document.querySelectorAll('.wallpaper-preset-swatch').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        state.wallpaperDraft.preset = btn.dataset.wallpaperPreset;
        if (btn.dataset.wallpaperPreset !== 'custom') state.wallpaperDraft.customUrl = '';
        fillWallpaperModal();
        persistWallpaperDraft();
      });
    });
    ['saturation', 'contrast', 'brightness', 'dim'].forEach(function (prop) {
      const input = $('wallpaper-' + prop);
      if (!input) return;
      input.addEventListener('input', function () {
        if (!state.wallpaperDraft) return;
        state.wallpaperDraft[prop] = Number(input.value);
        const label = $('wallpaper-' + prop + '-value');
        if (label) label.textContent = input.value + '%';
        updateWallpaperPreview();
      });
      input.addEventListener('change', persistWallpaperDraft);
    });
    const wallpaperUploadBtn = $('wallpaper-upload-btn'), wallpaperFileInput = $('wallpaper-file-input');
    if (wallpaperUploadBtn && wallpaperFileInput) wallpaperUploadBtn.addEventListener('click', function () { wallpaperFileInput.click(); });
    if (wallpaperFileInput) wallpaperFileInput.addEventListener('change', async function () {
      const file = wallpaperFileInput.files && wallpaperFileInput.files[0];
      wallpaperFileInput.value = '';
      if (!file || !state.wallpaperDraft) return;
      if (file.size > 8 * 1024 * 1024) { toast('A imagem deve ter no máximo 8 MB.', 'error'); return; }
      try {
        const uploaded = await window.WCMedia?.uploadForWallpaper?.(file);
        if (!uploaded || !uploaded.url) throw Error('Falha ao enviar imagem.');
        state.wallpaperDraft.preset = 'custom';
        state.wallpaperDraft.customUrl = uploaded.url;
        fillWallpaperModal();
        persistWallpaperDraft();
        toast('Papel de parede atualizado.', 'success');
      } catch (e) { toast(e.message || 'Falha ao enviar imagem.', 'error'); }
    });
    const wallpaperResetBtn = $('wallpaper-reset-btn');
    if (wallpaperResetBtn) wallpaperResetBtn.addEventListener('click', function () {
      if (!state.wallpaperEditingKey) return;
      state.wallpaperDraft = wallpaperDefaults();
      try { localStorage.removeItem('wc-wallpaper:' + state.wallpaperEditingKey); } catch (_) {}
      fillWallpaperModal();
      applyChatWallpaper(wallpaperKey());
      toast('Papel de parede restaurado.', 'success');
    });
    ...
    $('modal-settings-close')?.addEventListener('click', closeModals);
    ...
    if (el.messagesList) {
      el.messagesList.addEventListener('click', function (e) {
        const profileTarget = e.target.closest('.message-avatar, .message-author');
        if (profileTarget) {
          const item = profileTarget.closest('.message-item');
          const authorId = item && item.getAttribute('data-message-author-id');
          if (authorId) { window.WCFeatures?.profile?.(authorId); return; }
        }
    ...
    bindMobileNavAutoClose();
    bindFloatingPanelsAutoClose();
  }
