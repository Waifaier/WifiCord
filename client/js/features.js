...
// Delegado pro identity.js (única fonte de verdade pra moldura/
// decoração/efeito/aura — ver esse arquivo). `context` default 'profile'
// porque a maioria das chamadas daqui é o cartão de perfil cheio/popover;
// quem precisa de outro contexto (lista, chat) passa explicitamente.
function avatar(u, opts){
  if (window.WCIdentity) return window.WCIdentity.avatarHTML(u, Object.assign({ context: 'profile', size: 'lg' }, opts));
  ...
  return `<span class="profile-avatar-decorated">${inner}</span>`;
}
...
async function profile(id){
  try{
    ...
    const effectLayer=$('profile-effect-layer');if(effectLayer){
      ...
      // applyAtmosphere (identity.js) também liga o motor de partículas de
      // verdade (particle-engine.js) pra esse efeito quando o nível de
      // performance permite — em vez do truque de box-shadow sozinho.
      if(window.WCIdentity) window.WCIdentity.applyAtmosphere(effectLayer, effect, {speed:u.settings?.profileEffectSpeed||'normal'});
      else{effectLayer.className='profile-effect-layer profile-effect-'+effect+(effect!=='none'?' is-active':'');effectLayer.dataset.speed=u.settings?.profileEffectSpeed||'normal';effectLayer.innerHTML=effect!=='none'?'<span></span><span></span><span></span>':'';}
    }
