...
(function () {
  ...
  const ChatSocket = {
    ...
    connect() {
      ...
      this.socket.on('server-call:presence', d=>window.App?.handleServerCallPresence?.(d));
      this.socket.on('server:activity', d=>window.App?.handleServerActivity?.(d));
      this.socket.on('mention:new', d=>window.App?.handleMention?.(d));
    ...
    requestServerCallPresence(serverId, callback) { if(this.socket)this.socket.emit('server-call:presence-snapshot',{serverId},callback); },
