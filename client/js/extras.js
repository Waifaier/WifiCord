...
// companheiros) — mesmo padrão do #wc-frame-presets acima: guarda a escolha
// em window.__wcTempAvatarFx, só realmente gravada quando "Salvar perfil" é
// clicado (ver handler de #settings-save-profile logo abaixo).
...
document.getElementById('wc-profile-reset')?.addEventListener('click',()=>{
  ...
  for(let i=1;i<=3;i++){setVal(`settings-connection-${i}-name`,'');setVal(`settings-connection-${i}-url`,'')}window.__wcTempFrame='none';window.__wcTempAvatarFx='none';document.querySelectorAll('#wc-avatarfx-presets [data-avatarfx]').forEach(x=>x.classList.toggle('active',x.dataset.avatarfx==='none'));render();
});
