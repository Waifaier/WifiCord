...
router.put('/:serverId/transfer-owner', requireAuth, async (req, res, next) => {
  try {
    const serverId = parsePositiveInt(req.params.serverId);
    const newOwnerId = parsePositiveInt(req.body.newOwnerId);
    if (!serverId || !newOwnerId) {
      return res.status(400).json({ error: 'Dados inválidos.' });
    }
    if (!(await ServerModel.isOwner(serverId, req.session.userId))) {
      return res.status(403).json({ error: 'Somente o dono atual pode transferir o servidor.' });
    }
    if (Number(newOwnerId) === Number(req.session.userId)) {
      return res.status(400).json({ error: 'Você já é o dono deste servidor.' });
    }
    if (!(await ServerModel.isMember(serverId, newOwnerId))) {
      return res.status(400).json({ error: 'O novo dono precisa ser um membro deste servidor.' });
    }
    const server = await ServerModel.transferOwnership(serverId, newOwnerId);
    if (!server) {
      return res.status(404).json({ error: 'Servidor não encontrado.' });
    }
    req.app.get('io')?.to('server:' + serverId).emit('server:members:update', { serverId });
    req.app.get('io')?.to('server:' + serverId).emit('server:profile:update', { server: ServerModel.toPublic(server) });
