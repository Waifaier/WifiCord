...
// Guarda a última oferta de chamada 1:1 (SDP) ainda não atendida por
// destinatário, por até 45s. Serve pra quando a pessoa recebe o push de
// "ligação chegando" com o app fechado: ao reconectar o socket (o app
// abrindo), a gente reenvia essa mesma oferta como se tivesse acabado de
// chegar — reaproveitando 100% da tela de ligação que já existe no
// cliente, sem precisar de nenhuma UI nova pra esse caso.
const pendingCallOffers = new Map(); // userId (quem recebe) -> {fromUserId, sdp, callType, ts}
...
function initSockets(io) {
  // Usado pela rota REST /api/push/reject-call: quando a pessoa rejeita a
  // ligação direto pelo botão da notificação nativa (app nem chega a
  // abrir), isso avisa quem ligou e limpa a oferta pendente — sem isso o
  // chamador ficaria "chamando..." pra sempre.
  io.wcRejectPendingCall = function (calleeUserId, callerUserId) {
    pendingCallOffers.delete(calleeUserId);
    io.to(userRoom(callerUserId)).emit('call:hangup', { fromUserId: calleeUserId });
  };
  ...
  io.on('connection', (socket) => {
    ...
    // Reconectou (ex.: acabou de abrir o app a partir do push de ligação)
    // e tem uma oferta ainda válida esperando — reenvia ela agora, do
    // jeito que o cliente já sabe lidar (mesma tela de "fulano ligando").
    const pendingOffer = pendingCallOffers.get(userId);
    if (pendingOffer) {
      pendingCallOffers.delete(userId);
      if (Date.now() - pendingOffer.ts < PENDING_CALL_TTL_MS) {
    ...
    socket.on('call:offer', (data) => {
      ...
      const renegotiation = data.renegotiation === true;
      ...
      if (!renegotiation) {
        pendingCallOffers.set(toUserId, { fromUserId: userId, sdp: data.sdp, callType, ts: Date.now() });
        // Só acorda com push quem não tem nenhum socket conectado agora
        // (app fechado/travado) — se já está online, o evento acima já
        // mostra a telinha na hora, sem precisar de push nenhum.
        if (!onlineSockets.get(toUserId)?.size) {
          const caller = User.findById(userId);
          PushService.sendIncomingCallPush(toUserId, {
            fromUserId: userId,
            fromName: caller?.display_name || caller?.displayName || caller?.username,
            fromAvatar: caller?.avatar_url || caller?.avatarUrl || '',
            callType,
          });
        }
      }
    ...
    socket.on('call:hangup', (data) => {
      ...
      pendingCallOffers.delete(userId);
      pendingCallOffers.delete(toUserId);
    ...
    socket.on('server-call:join', (data, callback) => {
      ...
      const voiceLimit=Number(ServerModel.getSettings(serverId)?.voiceUserLimit)||0;
      if (voiceLimit > 0 && uniquePeers.length >= voiceLimit && !uniquePeers.includes(userId)) {
        return callback({ error: 'Este canal de voz atingiu o limite de ' + voiceLimit + ' pessoas.' });
      }
      socket.join(room); socket.data.serverCall={serverId,channelId,room};
      callback({ok:true,peers:uniquePeers});
