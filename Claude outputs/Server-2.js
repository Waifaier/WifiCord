...
const ServerModel = {
  ...
  updateProfile(serverId, { name, iconUrl, bannerUrl, backgroundUrl }) {
    db.prepare('UPDATE servers SET name=?, icon_url=?, banner_url=?, background_url=? WHERE id=?')
      .run(name, iconUrl ?? null, bannerUrl ?? null, backgroundUrl ?? null, serverId);
    return ServerModel.findById(serverId);
  },
  toPublic(server) { const settings=ServerModel.getSettings(server.id); return {id:server.id,name:server.name,ownerId:server.owner_id,inviteCode:server.invite_code,createdAt:server.created_at,iconUrl:server.icon_url||null,bannerUrl:server.banner_url||null,backgroundUrl:server.background_url||null,voiceUserLimit:settings.voiceUserLimit||0}; },
  // Transfere a posse do servidor para outro membro. O ex-dono continua
  // membro normal (perde o cargo "Admin" automatico, mas mantem qualquer
  // outro cargo que ja tinha).
  transferOwnership(serverId, newOwnerId) {
    const server = ServerModel.findById(serverId);
    if (!server) return null;
    if (!ServerModel.isMember(serverId, newOwnerId)) return null;
    db.prepare('UPDATE servers SET owner_id=? WHERE id=?').run(newOwnerId, serverId);
    const roles = defaultRoles(serverId);
    const adminRole = roles.find(r => r.name === 'Admin');
    if (adminRole) {
      db.prepare('INSERT OR IGNORE INTO server_member_roles(server_id,user_id,role_id) VALUES (?,?,?)').run(serverId, newOwnerId, adminRole.id);
    }
    return ServerModel.findById(serverId);
  },
};
